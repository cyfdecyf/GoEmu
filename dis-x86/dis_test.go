...
func TestPrefixParse(t *testing.T) {
	...
	testdata := []codeText{
		{[]byte{0x64, 0x8b, 0x35, 0x40, 0xce, 0x2f, 0xc0}, "mov %fs:0xc02fce40,%esi"},
		{[]byte{0xf0, 0x83, 0x04, 0x24, 0x00}, "lock addl $0x0,(%esp)"},
		{[]byte{0xf3, 0xab}, "rep stos %eax,%es:(%edi)"},
	}
	testDump(testdata, t)
}
...
func TestArith(t *testing.T) {
	testdata := []codeText{
		{[]byte{0x83, 0x60, 0x0c, 0xfe}, "andl $0xfffffffe,0xc(%eax)"},
		{[]byte{0x00, 0x00}, "add %al,(%eax)"},
		{[]byte{0x00, 0x45, 0xf3}, "add %al,-0xd(%ebp)"},
		{[]byte{0x02, 0x54, 0x28, 0xe5}, "add -0x1b(%eax,%ebp,1),%dl"},
		{[]byte{0x03, 0x05, 0x01, 0x00, 0x00, 0x00}, "add 0x1,%eax"},
		{[]byte{0x05, 0x32, 0x54, 0x12, 0x00}, "add $0x125432,%eax"},
		{[]byte{0x03, 0x45, 0x08}, "add 0x8(%ebp),%eax"},
		{[]byte{0x03, 0x04, 0x8d, 0x80, 0xa0, 0x2c, 0xc0}, "add -0x3fd35f80(,%ecx,4),%eax"},
	}
	testDump(testdata, t)
}
...
func TestMov(t *testing.T) {
	testdata := []codeText{
		{[]byte{0x0f, 0x21, 0xca}, "mov %db1,%edx"},
		{[]byte{0x0f, 0x20, 0xc2}, "mov %cr0,%edx"},
		{[]byte{0x66, 0x8c, 0xd0}, "mov %ss,%ax"},
		{[]byte{0x64, 0xa1, 0x40, 0xce, 0x2f, 0xc0}, "mov %fs:0xc02fce40,%eax"},
		{[]byte{0x8b, 0x1d, 0xa8, 0x6b, 0x25, 0xc0}, "mov 0xc0256ba8,%ebx"},
		{[]byte{0x88, 0x44, 0x3d, 0xd8}, "mov %al,-0x28(%ebp,%edi,1)"},
		{[]byte{0xb0, 0xeb}, "mov $0xeb,%al"},
		{[]byte{0xb9, 0x2f, 0x00, 0x00, 0x00}, "mov $0x2f,%ecx"},
		{[]byte{0xa0, 0x60, 0x96, 0x2c, 0xc0}, "mov 0xc02c9660,%al"},
		{[]byte{0xa1, 0x9c, 0xf6, 0x2b, 0xc0}, "mov 0xc02bf69c,%eax"},
		{[]byte{0xa3, 0x24, 0x01, 0x31, 0xc0}, "mov %eax,0xc0310124"},
		{[]byte{0x89, 0xd8}, "mov %ebx,%eax"},
		{[]byte{0x8a, 0x45, 0xec}, "mov -0x14(%ebp),%al"},
		{[]byte{0x8c, 0xd0}, "mov %ss,%eax"},
		{[]byte{0x8e, 0xd8}, "mov %eax,%ds"},
		{[]byte{0x8e, 0xd9}, "mov %ecx,%ds"},
	}
	testDump(testdata, t)
}
...
func TestTest(t *testing.T) {
	testdata := []codeText{
		{[]byte{0xf6, 0x86, 0x11, 0x02, 0x00, 0x00, 0x40}, "testb $0x40,0x211(%esi)"},
	}
	testDump(testdata, t)
}
...
func TestLgdt(t *testing.T) {
	testdata := []codeText{
		{[]byte{0x0f, 0x01, 0x15, 0xd2, 0xcd, 0x2b, 0x00}, "lgdtl 0x2bcdd2"},
	}
	testDump(testdata, t)
}
...
func TestLea(t *testing.T) {
	testdata := []codeText{
		{[]byte{0x8d, 0xa1, 0x00, 0x00, 0x00, 0x40}, "lea 0x40000000(%ecx),%esp"},
	}
	testDump(testdata, t)
}
...
func TestCall(t *testing.T) {
	testdata := []codeText{
		{[]byte{0xe8, 0x52, 0x9e, 0x0f, 0x00}, "call "},
		{[]byte{0xff, 0x15, 0x5c, 0xb7, 0x30, 0xc0}, "call *0xc030b75c"},
	}
	testDump(testdata, t)
}
